...
export default function SeatMapSystem() {
  ...
  // Hydration 에러 방지 및 로컬스토리지 로드
  useEffect(() => {
    setHasMounted(true);
    const saved = localStorage.getItem("office-map-v2");
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (hasMounted) {
      const data = { floors, teamNames, appTitle, adminPassword };
      localStorage.setItem("office-map-v2", JSON.stringify(data));
    }
  }, [floors, teamNames, appTitle, adminPassword, hasMounted]);
    ...
    <main style={mainContainerS}>
      ...
      <div style={sidebarS}>
        ...
        <div style={statsCardS}>
          <label style={labelS}>실시간 인원 통계</label>
        ...
        <div style={{ marginBottom: "20px" }}>
          <label style={labelS}>{isAdmin ? "층 관리" : "층 선택"}</label>
          {floors.map(f => (
          ...
          {isAdmin && <button onClick={() => setFloors([...floors, { id: `F${Date.now()}`, displayName: "새로운 층", items: [] }])} style={addFloorBtnS}>+ 층 추가</button>}
        </div>
        ...
        <div style={rightPanelS}>
          ...
          {selectedItem ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div style={propCardS}>
                <label style={labelS}>부서 및 색상</label>
                ...
                <input value={teamNames[selectedItem.color] || ""} onChange={(e) => setTeamNames({...teamNames, [selectedItem.color]: e.target.value})} style={inputS} placeholder="부서명 입력" />
                <div style={paletteS}>
              ...
              <div style={propCardS}>
                <label style={labelS}>정렬 및 복제</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5px" }}>
                  ...
                  <button onClick={() => updateItems(currentItems.map(i => selectedIds.includes(i.id) ? { ...i, rotation: (i.rotation + 90) % 360 } : i))} style={toolBtn}>90° 회전</button>
              ...
              <div style={propCardS}>
                ...
                <div style={{ display: "flex", gap: "5px" }}>
                  <div style={{flex:1}}><span style={{fontSize:"10px"}}>W</span><input type="number" value={selectedItem.width} onChange={(e) => updateItems(currentItems.map(i => selectedIds.includes(i.id) ? { ...i, width: +e.target.value } : i))} style={inputS} /></div>
                  <div style={{flex:1}}><span style={{fontSize:"10px"}}>H</span><input type="number" value={selectedItem.height} onChange={(e) => updateItems(currentItems.map(i => selectedIds.includes(i.id) ? { ...i, height: +e.target.value } : i))} style={inputS} /></div>
                </div>
              </div>
              <button onClick={() => { updateItems(currentItems.filter(i => !selectedIds.includes(i.id))); setSelectedIds([]); }} style={deleteBtnS}>선택 삭제</button>
            </div>
          ) : <div style={{ textAlign: "center", color: "#94a3b8", marginTop: "50px" }}>아이템을 선택하세요</div>}
        </div>
      )}
    </main>
  );
...
const floorInputS: any = { width: "65px", padding: "4px", border: "1px solid #e2e8f0", borderRadius: "5px", fontSize: "11px" };
...
const statsCardS: any = { padding: "12px", backgroundColor: "#f8fafc", borderRadius: "10px", marginBottom: "20px" };
...
const addFloorBtnS: any = { width: "100%", padding: "8px", border: "1px dashed #cbd5e1", background: "none", borderRadius: "8px", color: "#94a3b8", cursor: "pointer", fontSize: "11px" };
const smallBtnS: any = { padding: "0 10px", backgroundColor: "#f1f5f9", border: "none", borderRadius: "5px", fontSize: "11px", cursor: "pointer" };
const pwBtnS: any = { padding: "6px", background: "none", border: "1px solid #e2e8f0", borderRadius: "6px", color: "#64748b", fontSize: "11px", cursor: "pointer", marginBottom: "5px" };
