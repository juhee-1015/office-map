            ...
            <div>
              <div style={pcS}>
                ...
                {selItems[0].type==="seat"&&<>
                  <div style={{fontSize:"9px",color:"#94a3b8",marginBottom:"4px"}}>회전 <span style={{color:"#c0c8d6"}}>(Ctrl+←→)</span></div>
                  <div style={{display:"flex",gap:"3px",marginBottom:"8px"}}>
                    {[0,90,180,270].map(r=>(
                      <button key={r} onClick={()=>{saveHistory();updateItems(curItems.map(i=>selectedIds.includes(i.id)?{...i,rotation:r}:i));}}
                        style={{flex:1,padding:"5px 2px",border:`1.5px solid ${selItems[0].rotation===r?"#2563eb":"#e2e8f0"}`,borderRadius:"6px",fontSize:"10px",cursor:"pointer",
                          backgroundColor:selItems[0].rotation===r?"#eff6ff":"#fff",color:selItems[0].rotation===r?"#2563eb":"#64748b",fontWeight:700}}>
                        {r===0?"↑":r===90?"→":r===180?"↓":"←"}
                        <div style={{fontSize:"8px",color:"inherit",opacity:0.7}}>{r}°</div>
                      </button>
                    ))}
                  </div>
                </>}
                ...
                {selectedIds.length>1&&<>
                  <div style={{fontSize:"9px",color:"#94a3b8",marginBottom:"4px"}}>정렬</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"3px",marginBottom:"4px"}}>
                    {([
                      ...
                      ["↕","세로중앙",()=>{const ys=curItems.filter(i=>selectedIds.includes(i.id));const midY=(Math.min(...ys.map(i=>i.y))+Math.max(...ys.map(i=>i.y+i.height)))/2;saveHistory();updateItems(curItems.map(i=>selectedIds.includes(i.id)?{...i,y:midY-i.height/2}:i));}],
                      ...
                      ["↔","가로중앙",()=>{const xs=curItems.filter(i=>selectedIds.includes(i.id));const midX=(Math.min(...xs.map(i=>i.x))+Math.max(...xs.map(i=>i.x+i.width)))/2;saveHistory();updateItems(curItems.map(i=>selectedIds.includes(i.id)?{...i,x:midX-i.width/2}:i));}],
                    ] as [string,string,(()=>void)][]).map(([icon,lbl,fn])=>(
                      <button key={lbl} onClick={fn}
                        style={{padding:"5px 3px",border:"1px solid #e2e8f0",borderRadius:"6px",fontSize:"11px",cursor:"pointer",backgroundColor:"#f8fafc",color:"#475569",fontWeight:700,display:"flex",flexDirection:"column",alignItems:"center",gap:"1px"}}>
                        <span>{icon}</span>
                        <span style={{fontSize:"8px",color:"#94a3b8"}}>{lbl}</span>
                      </button>
                    ))}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px",marginBottom:"8px"}}>
                    {([
                      ["⇔","가로 균등 배분",()=>{const sel=curItems.filter(i=>selectedIds.includes(i.id)).sort((a,b)=>a.x-b.x);if(sel.length<3)return;saveHistory();const totalW=sel.reduce((s,i)=>s+i.width,0);const gap=(sel[sel.length-1].x+sel[sel.length-1].width-sel[0].x-totalW)/(sel.length-1);let cx=sel[0].x;updateItems(curItems.map(i=>{const idx=sel.findIndex(s=>s.id===i.id);if(idx<0)return i;const ni={...i,x:cx};if(idx<sel.length-1)cx+=sel[idx].width+gap;return ni;}));}],
                      ["⇕","세로 균등 배분",()=>{const sel=curItems.filter(i=>selectedIds.includes(i.id)).sort((a,b)=>a.y-b.y);if(sel.length<3)return;saveHistory();const totalH=sel.reduce((s,i)=>s+i.height,0);const gap=(sel[sel.length-1].y+sel[sel.length-1].height-sel[0].y-totalH)/(sel.length-1);let cy=sel[0].y;updateItems(curItems.map(i=>{const idx=sel.findIndex(s=>s.id===i.id);if(idx<0)return i;const ni={...i,y:cy};if(idx<sel.length-1)cy+=sel[idx].height+gap;return ni;}));}],
                    ] as [string,string,(()=>void)][]).map(([icon,lbl,fn])=>(
                      <button key={lbl} onClick={fn}
                        style={{padding:"5px",border:"1px solid #e2e8f0",borderRadius:"6px",fontSize:"10px",cursor:"pointer",backgroundColor:"#f8fafc",color:"#475569",fontWeight:700,display:"flex",alignItems:"center",gap:"4px",justifyContent:"center"}}>
                        <span style={{fontSize:"13px"}}>{icon}</span><span style={{fontSize:"9px"}}>{lbl}</span>
                      </button>
                    ))}
                  </div>
                </>}
