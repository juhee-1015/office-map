...
export default function SeatMapSystem() {
  ...
  useEffect(()=>{
    const h=(e:KeyboardEvent)=>{
      ...
      if(ctrl&&e.key==="d"){
        ...
        setUndoHistory(p=>[...p.slice(-29),snap]);
        const nids:number[]=[];
        const dd=curItems.filter(i=>selectedIds.includes(i.id)).map(i=>{
          const nid=Date.now()+Math.floor(Math.random()*99999);
          nids.push(nid);
          return{...i,id:nid,x:i.x+20,y:i.y+20};
        });
        setFloors(p=>p.map(f=>f.id===activeFloorId?{...f,items:[...f.items,...dd]}:f));
        setSelectedIds(nids);
        return;
      }
      if(ctrl&&e.key==="a"){
  ...
  },[isAdmin,undoHistory,selectedIds,selectedZoneId,curItems,curZones,activeFloorId]);
            ...
            return (
              <Draggable key={item.id} position={{x:item.x,y:item.y}}
                ...
                onStart={(e,_data)=>{
                  ...
                  if(me.shiftKey){
                    // Shift: 토글 선택 — 드래그는 막음
                    setSelectedIds(p=>p.includes(item.id)?p.filter(id=>id!==item.id):[...p,item.id]);
                    return false;
                  }
